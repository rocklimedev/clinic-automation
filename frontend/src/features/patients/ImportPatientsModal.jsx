...
export function ImportPatientsModal({ open, onClose }) {
  ...
  const [mapping, setMapping] = useState({ name: 'Patient', mobile: 'Phone', doctorName: 'Doctor', visitType: 'Type' })
  const queryClient = useQueryClient()

  const importMutation = useMutation({
    mutationFn: () => {
      const rows = SAMPLE_ROWS.map((r) => {
        const rowObj = Object.fromEntries(SAMPLE_HEADERS.map((h, i) => [h, r[i]]))
        return {
    ...
    >
        ...
        <div>
          <div
            ...
            className={`flex flex-col items-center justify-center rounded-xl border-2 border-dashed px-6 py-14 text-center transition-colors ${
              dragActive ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/10' : 'border-[rgb(var(--border-strong))]'
            }`}
          >
            ...
            <p className="text-sm font-medium text-[rgb(var(--fg))]">Drag and drop your file here</p>
            <p className="mt-1 text-xs text-[rgb(var(--muted-fg))]">Supports CSV and Excel (.xlsx) files up to 10MB</p>
            <label className="mt-4 cursor-pointer">
              <Button type="button" variant="secondary" size="sm" onClick={(e) => e.currentTarget.parentElement.querySelector('input').click()}>
                Browse files
              </Button>
              <input
                type="file"
      ...
      {step === 'mapping' && (
        <div className="space-y-5">
          <div className="flex items-center gap-2 rounded-lg border border-[rgb(var(--border))] px-3 py-2 text-sm">
            ...
            <span className="font-medium text-[rgb(var(--fg))]">{fileName || 'patients_import.csv'}</span>
          ...
          <div>
            <p className="mb-2 text-sm font-semibold text-[rgb(var(--fg))]">Map columns</p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {TARGET_FIELDS.map((field) => (
                <div key={field.key}>
                  <label className="mb-1 block text-xs font-medium text-[rgb(var(--muted-fg))]">{field.label}</label>
                  <Select
          ...
          <div>
            <p className="mb-2 text-sm font-semibold text-[rgb(var(--fg))]">Preview</p>
            <div className="overflow-x-auto rounded-lg border border-[rgb(var(--border))]">
              <table className="w-full text-xs">
                <thead className="bg-[rgb(var(--border))]/30">
                  <tr>
                    {SAMPLE_HEADERS.map((h) => (
                      <th key={h} className="px-3 py-2 text-left font-semibold text-[rgb(var(--muted-fg))]">
                        {h}
                      </th>
                    ))}
                  </tr>
      ...
      {step === 'success' && (
        <div className="flex flex-col items-center py-8 text-center">
          ...
          <p className="text-base font-semibold text-[rgb(var(--fg))]">Import complete</p>
          <p className="mt-1 max-w-sm text-sm text-[rgb(var(--muted-fg))]">
            {SAMPLE_ROWS.length} patients were added successfully and are now queued for the feedback automation.
          </p>
        </div>
      )}
    </Modal>
