...
const OPD_LOCATIONS = ['OPD 1 - Ground Floor', 'OPD 2 - First Floor', 'OPD 3 - Annex Wing', 'Main OPD']

export function PatientFormModal({ open, onClose }) {
  const queryClient = useQueryClient()
  const { data: doctors = [] } = useQuery({ queryKey: ['doctors'], queryFn: patientsApi.getDoctors })
  const { data: coordinators = [] } = useQuery({ queryKey: ['coordinators'], queryFn: patientsApi.getCoordinators })
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={handleSubmit((v) => createMutation.mutate(v))} disabled={isSubmitting || createMutation.isPending}>
            {createMutation.isPending ? 'Saving…' : 'Save patient'}
          </Button>
        </>
      }
    >
      <form className="space-y-8" onSubmit={handleSubmit((v) => createMutation.mutate(v))}>
        <section>
          <h3 className="text-sm font-semibold text-[rgb(var(--fg))]">Basic Information</h3>
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <Label required>Full Name</Label>
              <Input placeholder="e.g. Ananya Sharma" {...register('name')} error={errors.name} />
              {errors.name && <p className="mt-1 text-xs text-red-500">{errors.name.message}</p>}
            </div>
            <div>
              <Label required>Mobile Number</Label>
              <Input placeholder="10-digit number" maxLength={10} {...register('mobile')} error={errors.mobile} />
              {errors.mobile && <p className="mt-1 text-xs text-red-500">{errors.mobile.message}</p>}
            </div>
            <div>
              <Label>WhatsApp Number</Label>
              <Input placeholder="Same as mobile if left blank" maxLength={10} {...register('whatsapp')} />
            </div>
            <div>
              <Label>Email</Label>
              <Input type="email" placeholder="patient@example.com" {...register('email')} error={errors.email} />
        ...
        <section>
          <h3 className="text-sm font-semibold text-[rgb(var(--fg))]">Visit Details</h3>
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
            ...
            <div>
              ...
              <Input type="time" {...register('visitTime')} error={errors.visitTime} />
              {errors.visitTime && <p className="mt-1 text-xs text-red-500">{errors.visitTime.message}</p>}
            </div>
            <div>
              ...
              {errors.opdLocation && <p className="mt-1 text-xs text-red-500">{errors.opdLocation.message}</p>}
            </div>
            <div>
            ...
            <div>
              ...
              {errors.coordinator && <p className="mt-1 text-xs text-red-500">{errors.coordinator.message}</p>}
            </div>
            <div>
