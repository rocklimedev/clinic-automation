...
const FIRST_NAMES = [
  ...
  'Anika', 'Riya', 'Pari', 'Ira', 'Meera', 'Kabir', 'Aryan', 'Dhruv', 'Neha',
  'Priya', 'Sanya', 'Tara', 'Vikram', 'Rahul', 'Nikhil', 'Simran', 'Pooja',
  'Amit', 'Sunita', 'Rajesh', 'Deepak', 'Kiran', 'Manish', 'Shreya',
]
const LAST_NAMES = [
  'Sharma', 'Verma', 'Gupta', 'Malhotra', 'Kapoor', 'Iyer', 'Nair', 'Reddy',
  'Rao', 'Menon', 'Joshi', 'Chatterjee', 'Bose', 'Mehta', 'Agarwal', 'Singh',
  'Kumar', 'Patel', 'Desai', 'Pillai',
]
const DOCTORS = [
...
const COORDINATORS = [
  'Anjali Rawat', 'Suresh Pillai', 'Fatima Khan', 'Rakesh Yadav', 'Divya Menon',
]
...
export const coordinators = COORDINATORS.map((name, i) => ({ id: `coord-${i + 1}`, name }))
...
export const patients = Array.from({ length: 104 }).map((_, i) => {
  ...
  const feedbackReceived = (waStatus === 'read' || waStatus === 'delivered') && rand() > 0.4
  return {
    id: `PT-${1000 + i}`,
    name: `${first} ${last}`,
    gender: pick(GENDERS),
    ...
    email: rand() > 0.35 ? `${first.toLowerCase()}.${last.toLowerCase()}@example.com` : '',
    doctorId: doctor.id,
...
export const messageLogs = Array.from({ length: 132 }).map((_, i) => {
  ...
  return {
    ...
    status,
    deliveredAt: ['delivered', 'read'].includes(status) ? new Date(sentTime.getTime() + 1000 * 60 * 2).toISOString() : null,
    readAt: status === 'read' ? new Date(sentTime.getTime() + 1000 * 60 * 20).toISOString() : null,
    failReason: status === 'failed' ? pick(['Invalid number', 'User opted out', 'Session expired', 'Network error']) : null,
...
export function getDashboardStats() {
  ...
  const todaysPatients = patients.filter((p) => new Date(p.visitDate) >= today).length
  const sent = messageLogs.length
  const delivered = messageLogs.filter((m) => ['delivered', 'read'].includes(m.status)).length
...
export function getReviewConversion() {
  ...
  return [
    ...
    { name: 'No response', value: Math.max(stats.messagesDelivered - stats.feedbackReceived, 0) },
  ]
}
