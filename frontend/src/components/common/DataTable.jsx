        ...
        <table className="w-full text-sm">
          <thead>
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id} className="border-b border-[rgb(var(--border))] bg-[rgb(var(--border))]/20">
                {hg.headers.map((header) => {
                  const canSort = header.column.columnDef.enableSorting
                  const sortDir = sorting?.find((s) => s.id === header.column.id)?.desc
                  const isSorted = sorting?.some((s) => s.id === header.column.id)
                  return (
                    <th
                      key={header.id}
                      className={cn(
                        'whitespace-nowrap px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[rgb(var(--muted-fg))]',
                        canSort && 'cursor-pointer select-none hover:text-[rgb(var(--fg))]'
                      )}
                      onClick={() => canSort && onSortingChange?.(header.column.id)}
                    >
              ...
              table.getRowModel().rows.map((row) => (
                <tr
                  ...
                  className={cn(
                    ...
                    onRowClick && 'cursor-pointer hover:bg-[rgb(var(--border))]/25'
                  )}
                >
      ...
      {!loading && total > 0 && (
        <div className="flex flex-col items-center justify-between gap-3 border-t border-[rgb(var(--border))] px-4 py-3 sm:flex-row">
          <p className="text-xs text-[rgb(var(--muted-fg))]">
            Showing <span className="font-medium text-[rgb(var(--fg))]">{rangeStart}–{rangeEnd}</span> of{' '}
